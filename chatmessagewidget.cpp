...
ChatMessageWidget::ChatMessageWidget(QWidget *parent) : QWidget(parent)
{
    ...
    connect(m_stopLoadingTimer, &QTimer::timeout, this, &ChatMessageWidget::stopLoading);
}
...
void ChatMessageWidget::stopLoading()
{
...
void ChatMessageWidget::setText(QString text, QString time, QSize allSize, ChatMessageWidget::User_Type userType)
{
    ...
    // 随机生成头像索引
    QStringList heads = {":/img/avatar1.png", ":/img/avatar2.png"};
    ...
    // 设置 UserWidget 的信息
    m_userWidget->setInfo(userName, userTitle, QPixmap(headPath));
...
QSize ChatMessageWidget::fontRect(QString str)
{
    ...
    // 设置 UserWidget 的位置
    if (m_userType == User_Type::User_She) {
        m_userWidget->setGeometry(m_iconLeftRect.x(), m_iconLeftRect.bottom() + 5, m_iconLeftRect.width(), 40);
    } else if (m_userType == User_Type::User_Me) {
        m_userWidget->setGeometry(m_iconRightRect.x(), m_iconRightRect.bottom() + 5, m_iconRightRect.width(), 40);
    }
